layui.use(['jquery', 'table', 'laypage', 'layer'], function () {
    ...
    table.render({
        elem: '#table_user'
        ,
        url: '/staff/staffList?departmentId=' + $("#departmentId").val() + "&staffName=" + $("#staffName").val() + "&departUtilId=" + $("#departUtilId").val() + "&batchId=" + $("#batchId").val()
        ,
        cellMinWidth: 80
        ,
        cols: [[ //表头
    ...
    form.on('submit(add)', function (data) {
        ...
        $.ajax({
            type: "post"
            , url: "/staff/addStaffDo"
            , data: JSON.stringify({
                "id": data.field.id
                , "staffName": data.field.staffName
                , "staffNo": data.field.staffNo
    ...
    table.on('tool(table_demo)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            addOperation("编辑", "/staff/staffEdit?id=" + data.id + "&operate=detail");
            ...
            roleOperation("编辑", "/staff/staffEdit?id=" + data.id + "&operate=edit");
